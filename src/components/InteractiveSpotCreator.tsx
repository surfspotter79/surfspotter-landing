...
interface InteractiveSpotCreatorProps {
  onSpotCreate: (spotData: { name: string; coordinates: { lat: number; lng: number }; description: string }) => void;
  onClose: () => void;
}
...
const InteractiveSpotCreator: React.FC<InteractiveSpotCreatorProps> = ({ onSpotCreate, onClose }) => {
  ...
  const handleLocationSelect = useCallback((lat: number, lng: number) => {
    ...
    setSpotData(prev => ({ ...prev, coordinates }));
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    try {
      ...
      if (data && data.display_name) {
        ...
        if (suggestedName && !spotData.name) {
          setSpotData(prev => ({ ...prev, name: suggestedName }));
        }
      }
    } catch (error) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (spotData.name && spotData.coordinates.lat && spotData.coordinates.lng) {
      onSpotCreate(spotData);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Create New Surf Spot</h2>
            <button
              onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (lat && spotData.coordinates.lng) {
                          setSelectedLocation({ lat, lng: spotData.coordinates.lng });
                        }
                      }}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                      placeholder="-8.8290"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (spotData.coordinates.lat && lng) {
                          setSelectedLocation({ lat: spotData.coordinates.lat, lng });
                        }
                      }}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe the surf spot, wave type, conditions, access, etc..."
                  />
                </div>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-medium text-blue-900 mb-2">Tips for Creating Surf Spots:</h4>
                  <ul className="text-sm text-blue-800 space-y-1">
                    <li>• Click anywhere on the map to set the exact location</li>
                    <li>• Use the search bar to find known locations quickly</li>
                    <li>• Include wave type, best conditions, and access info in description</li>
                    <li>• Be respectful of local surf communities and environment</li>
                  </ul>
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={!spotData.name || !spotData.coordinates.lat || !spotData.coordinates.lng}
...
export default InteractiveSpotCreator;